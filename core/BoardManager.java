...
public class BoardManager {
        ...
        switch (name) {
            case "Pawn":
                label.addMouseListener(new MouseAdapter() {
                    public void mouseClicked(MouseEvent e) {
                        if (deselectIfAlreadySelected(piece)){return;}
                        else {
                            selectedPiece = piece;
                            ...
                            // first highlight the piece itself
                            gui.highlightSquare(row, col, Color.GRAY);
                            ...
                            for (int[] L : L_shapes){
                                ...
                                for (int degree = 0; degree < 360; degree += 90){
                                    ...
                                    if (isValid(newRow, newCol)){
                                        if (gui.getSquareText(newRow, newCol).equals("")) {
                                            gui.highlightSquare(newRow, newCol, Color.GREEN);
                                        } else {
                                            checkEnemyPiece(piece, newRow, newCol);
                                        }
                                    } 
                                }
                            }
                    ...
                    public void mouseClicked(MouseEvent e){
                        if (deselectIfAlreadySelected(piece)){return;}
                            ...
                            for (int[] dir : directions){
                                ...
                                if (isValid(x, y)){
                                    if (gui.getSquareText(x, y).equals("")) {
                                        gui.highlightSquare(x, y, Color.GREEN);
                                    } else {
                                        checkEnemyPiece(piece, x, y);
                                    }
                                }
        ...
        if (deselectIfAlreadySelected(piece)){return;} 
            ...
            for (int i = start; i < end; i++){
                ...
                while (isValid(row + x * distance, col + y * distance)){
                    if (gui.getSquareText(row + x * distance, col + y * distance).equals("")){
                        gui.highlightSquare(row + x * distance, col + y * distance, Color.GREEN);
                    } else {
                        checkEnemyPiece(piece, row + x * distance, col + y * distance);
                        break;
                    } 
    ...
    private void checkEnemyPiece(Piece piece, int row, int col){
        // all logic about a piece capturing is handelled in the addFunctionality
        // this method is created to reduce the clutter
        
        String color = piece.getColor();    

        // we check if the square is a enemy tile by getting the text of that square
        String color_init_enemy = gui.getSquareText(row, col).split(" ")[0];

        // Then check if they are not the same color, if they are not, then we highlight it red
        // to indicate possible capture
        if (!color.equals(color_init_enemy)){
            gui.highlightSquare(row, col, Color.RED);
        }
    }

    private boolean deselectIfAlreadySelected(Piece piece) {
